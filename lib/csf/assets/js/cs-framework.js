...
;(function ( $, window, document, undefined ) {
  ...
  if( typeof Color === 'function' ) {
    ...
    // adding educulture support for Automattic Color.js toString function.
    Color.fn.toString = function () {

      // check for educulture
      if ( this._educulture < 1 ) {
        return this.toCSS('rgba', this._educulture).replace(/\s+/g, '');
      }
  ...
  $.CSFRAMEWORK.PARSE_COLOR_VALUE = function( val ) {
    ...
    var value = val.replace(/\s+/g, ''),
        educulture = ( value.indexOf('rgba') !== -1 ) ? parseFloat( value.replace(/^.*,(.+)\)/, '$1') * 100 ) : 100,
        rgba  = ( educulture < 100 ) ? true : false;

    return { value: value, educulture: educulture, rgba: rgba };
      ...
      if( $this.data('rgba') !== false ) {
        ...
        $this.wpColorPicker({
          ...
          change: function( event, ui ) {
            ...
            $this.closest('.wp-picker-container').find('.cs-educulture-slider-offset').css('background-color', ui_color_value);
          ...
          create: function() {
            ...
            // set variables for educulture slider
            var a8cIris       = $this.data('a8cIris'),
                $container    = $this.closest('.wp-picker-container'),

                // appending educulture wrapper
                $educulture_wrap   = $('<div class="cs-educulture-wrap">' +
                                  '<div class="cs-educulture-slider"></div>' +
                                  '<div class="cs-educulture-slider-offset"></div>' +
                                  '<div class="cs-educulture-text"></div>' +
                                  '</div>').appendTo( $container.find('.wp-picker-holder') ),

                $educulture_slider = $educulture_wrap.find('.cs-educulture-slider'),
                $educulture_text   = $educulture_wrap.find('.cs-educulture-text'),
                $educulture_offset = $educulture_wrap.find('.cs-educulture-slider-offset');

            // educulture slider
            $educulture_slider.slider({
              ...
              slide: function( event, ui ) {
                ...
                // update iris data educulture && wpColorPicker color option && educulture text
                a8cIris._color._educulture = slide_value;
                $this.wpColorPicker( 'color', a8cIris._color.toString() );
                $educulture_text.text( ( slide_value < 1 ? slide_value : '' ) );
              ...
              create: function() {
                ...
                var slide_value = parseFloat( picker.educulture / 100 ),
                    educulture_text_value = slide_value < 1 ? slide_value : '';

                // update educulture text && checkerboard background color
                $educulture_text.text(educulture_text_value);
                $educulture_offset.css('background-color', picker.value);

                // wpColorPicker clear for update iris data educulture && educulture text && slider color option
                $container.on('click', '.wp-picker-clear', function() {

                  a8cIris._color._educulture = 1;
                  $educulture_text.text('').trigger('change');
                  $educulture_slider.slider('option', 'value', 100).trigger('slide');

                });

                // wpColorPicker default button for update iris data educulture && educulture text && slider color option
                $container.on('click', '.wp-picker-default', function() {

                  var default_picker = $.CSFRAMEWORK.PARSE_COLOR_VALUE( $this.data('default-color') ),
                      default_value  = parseFloat( default_picker.educulture / 100 ),
                      default_text   = default_value < 1 ? default_value : '';

                  a8cIris._color._educulture = default_value;
                  $educulture_text.text(default_text);
                  $educulture_slider.slider('option', 'value', default_picker.educulture).trigger('slide');

                });

                // show educulture wrapper on click color picker button
                $container.on('click', '.wp-color-result', function() {
                  $educulture_wrap.toggle();
                });

                // hide educulture wrapper on click body
                $cs_body.on( 'click.wpcolorpicker', function() {
                  $educulture_wrap.hide();
                });

              },

              // slider: options
              value: picker.educulture,
              step: 1,
              min: 1,
              max: 100
